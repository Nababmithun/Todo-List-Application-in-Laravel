...
function groupByDate(items) {
  ...
  // sort date keys: No due date last
  ...
  return (
    <AppLayout>
      ...
      <div className="flex items-center gap-3 mb-4">
        <Link href="/projects" className="px-3 py-1 rounded-full bg-slate-700 hover:bg-slate-600">&larr; Back</Link>
        <h1 className="text-xl font-semibold">{project?.name || `Project #${id}`}</h1>
      </div>
      ...
      <div className="bg-slate-800/70 rounded-2xl p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <select className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2"
                  value={onlyCompleted} onChange={e=>setOnlyCompleted(e.target.value)}>
            <option value="">All</option>
            <option value="true">Completed</option>
            <option value="false">Pending</option>
          </select>
          <button className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500" onClick={loadTasks}>Apply</button>
        </div>
      </div>
              ...
              <div key={dateKey} className="bg-slate-800/70 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-xs opacity-70">
                    Completed: {completed.length} • Pending: {pending.length} • Total: {list.length}
                  </div>
                </div>
